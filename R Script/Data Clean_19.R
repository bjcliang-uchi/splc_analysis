...
Edges_Data <- read_excel("~/Desktop/SPLC/SPLC_SAIC_061519.xlsx", sheet = "Edges")
# Edges Data
...
#Nodes Data
Nodes_Data <- read_excel("~/Desktop/SPLC/SPLC_SAIC_061519.xlsx", sheet = "Nodes")
Nodes_Data$id <- str_replace_all(Nodes_Data$Company," ","_")
Nodes_Data$Company=NULL
names(Nodes_Data)[names(Nodes_Data)=="market cap (USD)"] = "R"
Nodes_Data$size = substr(Nodes_Data$R,1,nchar(Nodes_Data$R)-1)
for (id in 1:length(Nodes_Data$R)){
  if(grepl('K',Nodes_Data$R[id])){
    Nodes_Data$size[id] = as.numeric(Nodes_Data$size[id])*1000
  }
  if(grepl('M',Nodes_Data$R[id])){
    Nodes_Data$size[id] = as.numeric(Nodes_Data$size[id])*1000000
  }
  if(grepl('B',Nodes_Data$R[id])){
    Nodes_Data$size[id] = as.numeric(Nodes_Data$size[id])*1000000000
  }
}
names(Nodes_Data)[names(Nodes_Data)=="Industry Group (GICS)"] = "type_label"
Nodes_Data=Nodes_Data[c('id',"country","type_label","size", "Type_color")]
...
#force factorize
Nodes_Data$size=as.numeric((Nodes_Data$size))
size = Nodes_Data$size
Nodes_Data$size[is.na(size)]=min(size[!is.na(size)]) #forcing NA to be min
Nodes_Data$size=log(Nodes_Data$size)
Nodes_Data$size = as.numeric(cut(Nodes_Data$size, breaks=7))
Nodes_Data$size =2*Nodes_Data$size 
