...
get_com = "=IFERROR(IF(M1<=0, update), \"\")"
...
for(i in 1:nrow(data)){
  ...
  #form$com[i] = gsub("update", data$company[i], get_com)
  #form$com[i] = gsub("1", i-(i %% repeat_t), form$com[i])
  ...
  form$count[i] = gsub("5", i, get_count)
  form$count[i] = gsub("fg", data$company[i], form$count[i])
}

for (i in seq(repeat_t+1, nrow(form), by=repeat_t)){
  form$com[i] = gsub("update", data$company[i], get_com)
  form$com[i] = gsub("1", i+1, form$com[i])
}

x = repeat_t+1:nrow(data)
y = seq(repeat_t+1, nrow(form), by=repeat_t)
z = x[!(x %in% y)]

for(i in z){
  a = (i-(i-1)%%5)+1
  form$com[i] = paste0("=A",a)
}

form$com[1]="company ticker"
write.csv(form, "data_access_update.csv",row.names=FALSE, na="")